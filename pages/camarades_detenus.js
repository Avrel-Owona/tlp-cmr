	...
	return (
		<>
			<HeadSeo
				...
				pageTitle={"Membres en danger"}
				previewImage={"/membre_en_danger/danger.png"}
			/>
			<div className="px-7 lg:px-20 pb-20">
				<PageHeader
					cover={"/2e_assemblee_generale_ordinaire/IMG_2CE9185DEAF3-1.jpeg"}
					title={
						"Mobilisons nous pour la libération des membres de Tournons La Page"
					}
				/>
			</div>
			<div className="flex px-7 lg:px-20 w-full sm:w-5/6 lg:w-10/12 xl:w-8/12 lg:py-20">
				<div className="text-3xl flex-col flex h-full sticky top-28">
					...
					>
						<FaFacebook
							round
							className="mb-4 text-3xl lg:text-4xl text-blue-500"
						/>
					</FacebookShareButton>
					<WhatsappShareButton
						url={`tournonslapagecameroun.org/camarades_detenus`}
					...
					>
						<FaWhatsapp
							round
							className="mb-4 text-3xl lg:text-4xl text-green-500"
						/>
					</WhatsappShareButton>
					<TwitterShareButton
						url={`tournonslapagecameroun.org/camarades_detenus`}
					...
					>
						<FaTwitter
							round
							className="mb-4 text-3xl lg:text-4xl text-sky-500"
						/>
					</TwitterShareButton>
				</div>
				<p className="ml-4 sm:ml-10 mb-5 lg:mb-0 text-base lg:text-2xl font-light text-gray-600 lg:text-gray-500">
					Le travail des activistes n'est pas sans danger et plusieurs subissent
					de graves violations de leurs droits. Les membres de Tournons La Page,
					par les sujets traités, sont particulièrement visés par ces attaques.
					Cette page a pour vocation d'attirer l'attention sur ces défenseurs
					des droits humains persécutés dans leurs pays respectifs pour leur
					travail de contrôle citoyen.
				</p>
			</div>
				...
				{MembresEnDanger.map((member, index) => {
					return (
						<MembreEnDangerCard
							...
							textOne={member.textOne}
							textTwo={member.textTwo}
						/>
					)
				})}
			</section>
			<section className="sm:p-20 p-7">
				<h1 className="text-5xl uppercase font-thin text-orange-500 mb-20 lg:block hidden">
					Qui sommes nous ?
				</h1>
				<div className="flex lg:bg-default w-full flex-wrap sm:justify-between lg:justify-center items-center lg:py-20">
					<div className="w-full text-left mt-10 lg:mt-0 sm:w-72 lg:w-96 xl:mr-20 flex flex-col justify-center">
						<h3 className="bg-white lg:p-14 lg:pb-20 text-gray-700 font-light text-base lg:text-xl">
							<span className="text-orange-500">Tournons La Page Cameroun</span>
							<br /> ( TLP-CMR ) est implanté dans 10 régions, 58 départements,
							et 361 arrondissements.
						</h3>
					...
					<img
						src="/afrique-carte.png"
						className="sm:mt-0 w-full sm:w-3/6 mt-10 lg:w-3/6 xl:w-96 h-full object-cover z-0"
						alt="carte"
					/>
				</div>
			</section>
