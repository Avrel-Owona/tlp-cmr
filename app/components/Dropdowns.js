...
const Dropdowns = () => {
    ...
    return (
        <div className='w-64 absolute top-48 z-40 right-10 shadow-md'>
            <ul className={dropdown ? 'bg-white text-black flex flex-col' : 'hidden'}  onClick={()=>setDropdown(!dropdown)}>
                {DropdownsItems.map((item, index)=> {
                    return (
                        <ActiveLink key={index} activeClassName="font-semibold text-orange-600" href={item.pathname}>
                            <li
